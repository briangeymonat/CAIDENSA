...
namespace Persistencia.Clases
{
    public class pSesion : pPersistencia
    {
        ...
        {
            cUsuarioSesion retorno = new cUsuarioSesion();
            try
            {
                var conn = new SqlConnection(CadenaDeConexion);
                conn.Open();

                SqlCommand cmd = new SqlCommand("UsuariosSesiones_TraerObservacionPorUsuarioYSesion", conn);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                using (SqlDataReader oReader = cmd.ExecuteReader())
                {
                    while (oReader.Read())
                    {
                        retorno.Usuario = parUsuarioSesion.Usuario;
                        retorno.Sesion = parUsuarioSesion.Sesion;
                        retorno.Observacion = oReader["UsuariosSesionesObservacion"].ToString();
                    }
                    conn.Close();
                }
            }
        ...
        public static List<DateTime> TraerMaximaFechaYMinimaFecha()
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("Sesiones_MaxFechaYMinFecha", conn);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                using (SqlDataReader oReader = cmd.ExecuteReader())
                {
                    ...
                    {
                        fecha = new DateTime();
                        fecha = DateTime.Parse(oReader["maximo"].ToString());
                        retorno.Add(fecha);
                        fecha = new DateTime();
                        fecha = DateTime.Parse(oReader["minimo"].ToString());
                        retorno.Add(fecha);
                    }
                    conn.Close();
                }
        ...
        public static List<string> TraerAsistenciasDeBeneficiarioPorMes(cBeneficiario parBeneficiario, string parAno, string parMes)
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("BeneficiariosSesiones_AsistenciasPorMes", conn);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                using (SqlDataReader oReader = cmd.ExecuteReader())
                {
                    while (oReader.Read())
                    {
                        dia = DateTime.Parse(oReader["SesionFecha"].ToString()).Day - 1;
                        ...
                        if (estado != "NO_AGREGAR")
                        {
                            if (retorno[dia] == null || retorno[dia] == string.Empty)
                            {
                                retorno[dia] = estado;
                            }
                            else
